...
export const ReportsPage = ({ sales, productionReports, users, stockHistory, productionRequests, inventory }: { sales: Sale[]; productionReports: ProductionReport[]; users: UserData[], stockHistory: StockHistoryEntry[], productionRequests: ProductionRequest[], inventory: InventoryItem[] }) => {
    ...
    const renderContent = () => {
        switch(view) {
            ...
            case 'warehouse': return <WarehouseReportView stockHistory={stockHistory} productionRequests={productionRequests} inventory={inventory} users={users} />;
            default: return null;
        }
    }
...
const WarehouseReportView = ({ stockHistory, productionRequests, inventory, users }: { stockHistory: StockHistoryEntry[], productionRequests: ProductionRequest[], inventory: InventoryItem[], users: UserData[] }) => {
    const { showPrintPreview } = usePrintPreview();
    return (
        <div className="space-y-6">
            <Card>
