...
import type { Material, ProductionReport, UserData, GarmentOrderItem, AdditionalCost, MaterialPriceUnit, StockHistoryType, AllSizingStandards, ProductionRequest, ProductionRequestStatus, FinishedGood } from '../types';
...
const getInitialOrderState = (sizingStandards: AllSizingStandards): GarmentOrderItem[] => {
    const initialGarmentKey = Object.keys(GARMENT_PATTERNS)[0];
    const garmentInfo = GARMENT_PATTERNS[initialGarmentKey];
...
export const ProductionPage = ({ materials, productionReports, setProductionReports, currentUser, addActivity, updateStock, sizingStandards, productionRequests, setProductionRequests, users, finishedGoods }: ProductionPageProps) => {
    const [view, setView] = useState<'calculator' | 'requests'>('calculator');
    const [selectedGarment, setSelectedGarment] = useState(Object.keys(GARMENT_PATTERNS)[0]);
    
    const [materialPriceUnit, setMaterialPriceUnit] = useState<MaterialPriceUnit>('kg');
    const [materialPrice, setMaterialPrice] = useState(materials.find(m => m.id === GARMENT_PATTERNS[selectedGarment].materialId)?.pricePerUnit || 0);
    const [order, setOrder] = useState<GarmentOrderItem[]>(() => getInitialOrderState(sizingStandards));
    const [additionalCosts, setAdditionalCosts] = useState<AdditionalCost[]>([{ id: crypto.randomUUID(), name: 'Benang & Label', cost: 2500 }]);
    ...
    const selectedGarmentInfo = useMemo(() => GARMENT_PATTERNS[selectedGarment], [selectedGarment]);

    const resetFormForNewGarment = (garmentKey: string) => {
        setSelectedGarment(garmentKey);
        setHppResult(null);
        setFulfillingRequestId(null);
        const newGarmentInfo = GARMENT_PATTERNS[garmentKey];
    ...
    const handleFulfillRequest = (request: ProductionRequest) => {
        ...
        const garmentKey = Object.keys(GARMENT_PATTERNS).find(key => 
            firstProduct.name.toLowerCase().includes(GARMENT_PATTERNS[key].title.toLowerCase())
        );
                ...
                <div className="bg-white p-6 rounded-xl shadow-md space-y-6">
                     ...
                     <div>
                         ...
                         <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                             {Object.entries(GARMENT_PATTERNS).map(([key, value]) => {
                                 ...
                                 return (
                                     ...
                                     >
                                         ...
                                         <span className="text-xs font-semibold">{value.title}</span>
                                     </button>
                                 )
                             })}
                         </div>
                     ...
                     <div>
                         ...
                         <div className="flex">
                             ...
                             <CustomInput type="number" placeholder="Contoh: 150000" value={materialPrice} onChange={e => setMaterialPrice(Number(e.target.value))} className="rounded-none"/>
                        ...
                        <div className="space-y-2">
                           <AnimatePresence>
                            {order.map((item, index) => {
                                const itemAvailableSizes = Object.keys(sizingStandards[selectedGarment] || {});
                                return (
                                <motion.div 
                                    key={item.id} 
                                    ...
                                    <div className="grid grid-cols-12 gap-2 items-end">
                                        {selectedGarmentInfo.models.length > 0 ? (
                                            <>
                                                ...
                                                <div className="col-span-3">
                                                    <CustomInput label={index === 0 ? 'Jumlah' : ''} type="number" value={item.quantity} onChange={e => handleOrderChange(item.id, 'quantity', parseInt(e.target.value, 10) || 0)} />
                                                </div>
                                            </>
                                        ) : (
                                            <>
                            ...
                            <AnimatePresence>
                                {additionalCosts.map(item => (
                                    <motion.div key={item.id} variants={itemVariants} initial="hidden" animate="visible" exit="exit" layout className="flex gap-2 items-end">
                                        ...
                                        <CustomInput type="number" placeholder="Biaya" value={item.cost} onChange={e => handleCostChange(item.id, 'cost', parseInt(e.target.value, 10) || 0)} />
    ...
    const renderWarehouseRequests = () => {
        ...
        const approvedRequests = productionRequests.filter(r => r.status === 'approved_by_production');
        const otherRequests = productionRequests.filter(r => r.status !== 'pending' && r.status !== 'approved_by_production').sort((a,b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
        ...
        // FIX: Explicitly type RequestCard with React.FC and a props interface to ensure TypeScript
        // correctly identifies it as a component that accepts a `key` prop within a map.
             ...
             <div className="bg-white p-4 rounded-xl shadow-md border">
                ...
                <div className="flex gap-2 justify-end mt-4">
                    {request.status === 'pending' && (
                        <Button size="sm" variant="primary" onClick={() => handleApproveRequest(request.id)}>
                            <ThumbsUp size={14} className="mr-1"/> Setujui & Proses
                        </Button>
                    )}
                     {request.status === 'approved_by_production' && (
                        <Button size="sm" variant="primary" className="bg-green-600 hover:bg-green-700" onClick={() => handleFulfillRequest(request)}>
