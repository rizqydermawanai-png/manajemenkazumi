...
import { Plus, Check, Truck, X, Printer, Edit,ThumbsUp, ThumbsDown, Trash2, AlertTriangle, AlertCircle, PackageCheck, PlusCircle, Send, Package, CheckCircle2, Wallet } from 'lucide-react';
...
type WarehouseView = 'materials' | 'finished' | 'requests' | 'verification' | 'outgoing' | 'history';
type AdjustmentType = 'material' | 'finishedGood';

// New component for stock status indicator
const StockStatusIndicator = ({ stock, type }: { stock: number; type: 'material' | 'finishedGood' }) => {
    const lowStockThreshold = type === 'material' ? 20 : 10;
...
export const WarehousePage = ({ 
    ...
    setOnlineOrders, stockHistory, updateStock, addStockHistory, receiveProductionGoods,
    stockAdjustments, setStockAdjustments, currentUser, users, productionRequests, setProductionRequests, onDispatchOrder
}: WarehousePageProps & { users: UserData[] }) => {
    const [view, setView] = useState<WarehouseView>('outgoing');
    ...
    const pendingAdjustmentsCount = stockAdjustments.filter(adj => adj.status === 'pending').length;
    ...
    const handleUpdateOrderStatus = (orderId: string, status: OnlineOrderStatus) => {
        setOnlineOrders(prev => prev.map(o => o.id === orderId ? {
            ...o,
            status,
            history: [...o.history, { status, timestamp: new Date().toISOString(), userId: currentUser.uid }]
        } : o));
        
        if (status === 'pending_gudang') {
            addActivity('Gudang', `Mengkonfirmasi pembayaran untuk pesanan #${orderId}`, orderId);
            addToast({ title: 'Pembayaran Dikonfirmasi', message: 'Pesanan sekarang siap untuk diproses.', type: 'success' });
        }
    };
        ...
        reader.onloadend = async () => {
            if (typeof reader.result === 'string') {
                try {
                    ...
                    setEditingGood(prev => prev ? { ...prev, imageUrl: data.url } : null);
                } catch (error) {
    ...
    const renderContent = () => {
        switch(view) {
            case 'materials': return <MaterialsView materials={materials} onAddClick={() => setIsMaterialModalOpen(true)} onPrintChecklist={handlePrintMaterialChecklist} onAdjustClick={() => openAdjustmentModal('material')} />;
            ...
            case 'requests': return <ProductionRequestView finishedGoods={finishedGoods} currentUser={currentUser} addActivity={addActivity} setProductionRequests={setProductionRequests} productionRequests={productionRequests} users={users} />;
            ...
            case 'outgoing': return <OutgoingProductsView orders={onlineOrders} onUpdateStatus={handleUpdateOrderStatus} onDispatchOrder={onDispatchOrder} />;
            case 'history': return <StockHistoryView history={stockHistory} />;
            default: return null;
        }
    };
    
    const warehouseViews: { key: WarehouseView; label: string; adminOnly: boolean }[] = [
        { key: 'materials', label: 'Material Mentah', adminOnly: false },
        { key: 'finished', label: 'Barang Jadi', adminOnly: false },
        { key: 'requests', label: 'Permintaan Produksi', adminOnly: false },
        { key: 'verification', label: 'Verifikasi Stok', adminOnly: true },
        { key: 'outgoing', label: 'Produk Keluar', adminOnly: false },
        { key: 'history', label: 'Riwayat Stok', adminOnly: false },
    ];
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-white p-2 rounded-xl shadow-md flex gap-2 flex-wrap">
                {warehouseViews.filter(v => !v.adminOnly || isSuperAdmin).map(v => (
                    <Button key={v.key} variant={view === v.key ? 'primary' : 'ghost'} onClick={() => setView(v.key)} className="flex-1 relative">
                        {v.label}
                        {v.key === 'verification' && pendingAdjustmentsCount > 0 && (
                            <span className="absolute -top-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full bg-red-500 text-xs font-bold text-white">{pendingAdjustmentsCount}</span>
                        )}
                    </Button>
                ))}
            </div>
                ...
                {editingGood && (
                    <div className="space-y-4">
                        ...
                        <div className="flex flex-col items-center gap-2">
                            <img 
                                src={editingGood.imageUrl || 'https://placehold.co/150x150/e2e8f0/64748b?text=Foto'} 
                                alt="Preview" 
...
const MaterialsView = ({ materials, onAddClick, onPrintChecklist, onAdjustClick }: { materials: Material[], onAddClick: () => void, onPrintChecklist: () => void, onAdjustClick: () => void }) => (
    <div className="space-y-4">
        ...
        <div className="bg-white rounded-xl shadow-md overflow-hidden">
            <div className="overflow-x-auto">
                <table className="w-full text-left min-w-[600px]"><thead className="bg-slate-50 border-b"><tr><th className="p-4 font-semibold">Nama Material</th><th className="p-4 font-semibold">Stok</th><th className="p-4 font-semibold">Satuan</th><th className="p-4 font-semibold">Harga/Satuan</th><th className="p-4 font-semibold">Nilai Stok</th></tr></thead><tbody>{materials.map(mat => (<tr key={mat.id} className="border-b last:border-0 hover:bg-slate-50"><td className="p-4">{mat.name}</td><td className="p-4"><StockStatusIndicator stock={mat.stock} type="material" /></td><td className="p-4">{mat.unit}</td><td className="p-4">{formatCurrency(mat.pricePerUnit)}</td><td className="p-4 font-semibold">{formatCurrency(mat.stock * mat.pricePerUnit)}</td></tr>))}</tbody></table>
            </div>
        </div>
    </div>
);

const FinishedGoodsView = ({ reports, finishedGoods, onReceive, onPrintChecklist, onAdjustClick, currentUser, onEditClick }: { reports: ProductionReport[], finishedGoods: FinishedGood[], onReceive: (report: ProductionReport) => void, onPrintChecklist: () => void, onAdjustClick: () => void, currentUser: UserData, onEditClick: (good: FinishedGood) => void }) => {
    ...
    }, [finishedGoods, stockFilter]);

    return (
        <div className="space-y-6">
            <div>
                <h2 className="text-xl font-bold text-slate-700 mb-2">Barang Masuk dari Produksi</h2>
                    ...
                    <table className="w-full text-left text-sm min-w-[700px]">
                        ...
                        <tbody>
                            {filteredGoods.map(good => (
                                <tr key={good.id} className="border-b last:border-0 hover:bg-slate-50">
                                    <td className="p-2">
                                        <img src={good.imageUrl || `https://placehold.co/40x40/e2e8f0/64748b?text=KZM`} alt={good.name} className="w-10 h-10 rounded-md object-cover bg-slate-100"/>
                                    </td>
                                    <td className="p-4">{good.name} - {good.size} ({good.colorName})</td>
                                    <td className="p-4"><StockStatusIndicator stock={good.stock} type="finishedGood" /> pcs</td>
...
const ProductionRequestView = ({ finishedGoods, currentUser, addActivity, setProductionRequests, productionRequests, users }: { finishedGoods: FinishedGood[], currentUser: UserData, addActivity: (type: string, description: string, relatedId?: string) => void, setProductionRequests: React.Dispatch<React.SetStateAction<ProductionRequest[]>>, productionRequests: ProductionRequest[], users: UserData[] }) => {
    ...
    const criticallyLowStockGoods = useMemo(() => finishedGoods.filter(g => g.stock > 0 && g.stock < CRITICAL_STOCK_THRESHOLD), [finishedGoods]);
    const lowStockGoods = useMemo(() => finishedGoods.filter(g => g.stock >= CRITICAL_STOCK_THRESHOLD && g.stock < LOW_STOCK_THRESHOLD), [finishedGoods]);
    ...
    const hasRecommendations = outOfStockGoods.length > 0 || criticallyLowStockGoods.length > 0 || lowStockGoods.length > 0;
            ...
            <div>
                ...
                {hasRecommendations ? (
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <RecommendationCard title="Stok Kritis" icon={<AlertTriangle size={20} className="mr-2 text-orange-500"/>} products={criticallyLowStockGoods} onAdd={addItemToRequestForm} requestItems={requestItems} />
                        <RecommendationCard title="Stok Sedikit" icon={<AlertTriangle size={20} className="mr-2 text-yellow-500"/>} products={lowStockGoods} onAdd={addItemToRequestForm} requestItems={requestItems} />
                    </div>
                ) : (
                    <div className="text-center p-8 text-slate-500 bg-white rounded-xl shadow-lg border">
...
const VerificationView = ({ adjustments, onReview, users }: { adjustments: StockAdjustment[], onReview: (id: string, decision: 'approve' | 'reject') => void, users: UserData[] }) => {
    ...
    <div className="space-y-4">
        ...
        )) : <div className="text-center p-12 text-slate-500 bg-white rounded-xl shadow-lg border"><Check size={48} className="mx-auto text-slate-400 mb-2" /><p>Tidak ada permintaan verifikasi stok.</p></div>}
    </div>
)};

const OutgoingProductsView = ({ orders, onUpdateStatus, onDispatchOrder }: { orders: OnlineOrder[], onUpdateStatus: (orderId: string, status: OnlineOrderStatus) => void, onDispatchOrder: (order: OnlineOrder, trackingNumber: string) => void }) => {
    const [isProofModalOpen, setIsProofModalOpen] = useState<string | null>(null);
    ...
    const getStatusInfo = (status: OnlineOrderStatus) => {
        const info: {[key in OnlineOrderStatus]: {text: string, color: string, icon: React.ReactNode}} = {
            ...
            'pending_gudang': { text: 'Menunggu Konfirmasi', color: 'bg-yellow-100 text-yellow-800', icon: <Package size={14}/> },
    ...
    const formatAddress = (address: Address) => {
        ...
        return `${address.streetAndBuilding}, Kel. ${address.subdistrict}, Kec. ${address.district}, ${address.city}, ${address.province}, ${address.postalCode}`;
    };
    
    const paymentPendingOrders = orders.filter(o => o.status === 'pending_payment');
    const activeOrders = orders.filter(o => o.status !== 'selesai' && o.status !== 'dibatalkan' && o.status !== 'pending_payment');

    return (
        <div className="space-y-6">
            <Modal isOpen={!!isProofModalOpen} onClose={() => setIsProofModalOpen(null)} title="Bukti Pembayaran">
                <img src={isProofModalOpen || ''} alt="Bukti Pembayaran" className="w-full h-auto rounded-lg"/>
            </Modal>
            
            <div>
                <h2 className="text-xl font-bold text-slate-700 mb-2">Menunggu Konfirmasi Pembayaran</h2>
                <div className="space-y-4">
                {paymentPendingOrders.length > 0 ? paymentPendingOrders.map(order => (
                    <div key={order.id} className="bg-white p-4 rounded-xl shadow-md border border-orange-300">
                        ...
                        <div className="flex justify-between items-center">
                             {order.paymentProofUrl && (
                                <img 
                                    ...
                                    onClick={() => setIsProofModalOpen(order.paymentProofUrl || null)}
                                />
                            )}
                            <Button size="sm" variant="primary" onClick={() => onUpdateStatus(order.id, 'pending_gudang')}>
                                <Check size={14}/> Konfirmasi Pembayaran
                            </Button>
                        </div>
                    </div>
                )) : <div className="text-center p-12 text-slate-500 bg-white rounded-xl shadow-lg border"><Wallet size={48} className="mx-auto text-slate-400 mb-2" /><p>Tidak ada pesanan yang menunggu pembayaran.</p></div>}
                </div>
            </div>

            <div>
                <h2 className="text-xl font-bold text-slate-700 mb-2">Pesanan Online Aktif</h2>
                <div className="space-y-4">
                    ...
                    <div key={order.id} className="bg-white p-4 rounded-xl shadow-md border">
                        ...
                        <div className="flex flex-col sm:flex-row gap-2 justify-end mt-4">
                            {order.status === 'pending_gudang' && (
                                <Button size="sm" variant="primary" onClick={() => onUpdateStatus(order.id, 'approved_gudang')}>
                                    <Check size={14}/> Setujui & Siapkan
                                </Button>
                            )}
                ...
                )) : <div className="text-center p-12 text-slate-500 bg-white rounded-xl shadow-lg border"><Package size={48} className="mx-auto text-slate-400 mb-2" /><p>Tidak ada pesanan online yang perlu diproses.</p></div>}
                </div>
            </div>
        </div>
    );
